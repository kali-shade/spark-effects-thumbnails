 ...
 * @param {string} effect - The effect type (Slices, Magic, Spread, Unroll, Twister, Rain, or Pieces)
 */
async function loadSplatForEffect(effect) {
  ...
  if (effect === "Magic") {
    ...
    splatFileName = "tomahawk-niku.spz";
    position = [0, 0, 0];
  }
      ...
      const d = new dyno.Dyno({
        ...
        statements: ({ inputs, outputs }) =>
          ...
          if (${inputs.effectType} == 1) {
            ...
            // Slices Effect: Horizontal slices (vertical lines) come from back to front
            float sliceWidth = 0.3;
            float sliceIndex = floor(localPos.z / sliceWidth);
            
            // Calculate delay: each slice appears sequentially from back to front
            // Use sliceIndex to create unique delays for each slice
            // Back slices (lower sliceIndex, more negative Z) appear first
            float maxExpectedSlice = 20.0;
            float minExpectedSlice = -20.0;
            float normalizedSlice = (sliceIndex - minExpectedSlice) / (maxExpectedSlice - minExpectedSlice);
            normalizedSlice = clamp(normalizedSlice, 0.0, 1.0);
            
            // Invert: back side (lower normalizedSlice) has lower delay
            // Multiply by delay per slice to space them out
            float delayPerSlice = 0.25; // Time between each slice appearance
            float delay = normalizedSlice * (maxExpectedSlice - minExpectedSlice) * delayPerSlice;
            
            float sliceTime = max(0.0, t - delay + 2.0);
            
            // Animation duration for each slice
            float animationDuration = 1.2;
            float progress = clamp(sliceTime / animationDuration, 0.0, 1.0);
            
            // Easing function for smooth movement
            float eased = progress * progress * (3.0 - 2.0 * progress);
            
            // Starting position: offset to the back and slightly above
            float offsetDistance = -15.0;
            float verticalOffset = -10.0; // Offset upward
            vec3 startPos = localPos + vec3(0.0, verticalOffset, -offsetDistance);
            
            // Interpolate from start position to original position
            vec3 finalPos = mix(startPos, localPos, eased);
            
            ${outputs.gsplat}.center = finalPos;
            
            // Scale animation: start small, grow to normal
            float scaleProgress = clamp(sliceTime / (animationDuration * 0.6), 0.0, 1.0);
            float scaleEased = scaleProgress * scaleProgress;
            vec3 finalScales = mix(scales * 0.1, scales, scaleEased);
            ${outputs.gsplat}.scales = finalScales;
            
            // Color interpolation: white to original color
            vec3 whiteColor = vec3(1.0, 1.0, 1.0);
            vec3 originalColor = ${inputs.gsplat}.rgba.rgb;
            vec3 finalColor = mix(whiteColor, originalColor, smoothstep(.99,1.,eased));
            
            // Opacity: fade in as slice arrives
            float opacityProgress = clamp(sliceTime / (animationDuration * 2.), 0.0, 1.0);
            float alpha = ${inputs.gsplat}.rgba.a * opacityProgress;
            ${outputs.gsplat}.rgba = vec4(finalColor, alpha);
          }
        `),
      });
